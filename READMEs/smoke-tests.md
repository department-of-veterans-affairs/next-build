...
The CMS Team will be notified via slack alert ([example](https://dsva.slack.com/archives/C06DSBT7CBW/p1771529468872979)) when the deployment has successfully completed in Staging.
...
## Phase 1 - Manual Smoke Test (began Feb 17, 2026)
...
- We will manually compare the identified critical URLs in Staging to the same pages in Production.
  ...
  - If any issues are detected, CMS engineering support will be consulted to determine path of least resistance to unblock the deployment to Production. (i.e. revert related PR or introduce quick fix)
    - In some cases, react app content will be different between Staging and Production due to feature toggles. This is expected and should not prevent Next Build deployment.
