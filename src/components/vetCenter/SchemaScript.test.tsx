...
import { VetCenter } from './formatted-type'
...
describe('SchemaScript', () => {
  describe('Place schema (main structured data)', () => {
    test('renders Place schema script tag with correct structure', () => {
      const { container } = render(<SchemaScript vetCenter={mockData} />)
    ...
    test('includes address_line2 in streetAddress when present', () => {
      ...
      const { container } = render(
        <SchemaScript vetCenter={dataWithAddressLine2} />
      )
    ...
    test('handles missing image gracefully', () => {
      ...
      const { container } = render(
        <SchemaScript vetCenter={dataWithoutImage as VetCenter} />
      )
    ...
    test('maps office hours correctly to schema.org format', () => {
      ...
      // Find Monday (day 1) entry - dayOfWeek format is https://schema.org/{dayNumber}
  ...
  describe('GovernmentService schema (health services)', () => {
    test('renders GovernmentService schema for each health service', () => {
      ...
      // Should have 1 Place schema + health services schemas
      expect(scriptTags.length).toBe(1 + mockData.healthServices.length)

      // Get all GovernmentService schemas (skip first Place schema)
    ...
    test('includes serviceLocation with Place information', () => {
      ...
      const serviceLocation =
        firstServiceSchema.availableChannel.serviceLocation
    ...
    test('includes geo coordinates in serviceLocation', () => {
      ...
      const geo = firstServiceSchema.availableChannel.serviceLocation.geo
    ...
    test('handles alternateName when vetCenterFriendlyName is present', () => {
      ...
      const { container } = render(
        <SchemaScript vetCenter={dataWithFriendlyName} />
      )
    ...
    test('handles empty health services array', () => {
      ...
      const { container } = render(
        <SchemaScript vetCenter={dataWithoutHealthServices} />
      )
      ...
      // Should only have the Place schema
  ...
  describe('Edge cases', () => {
    test('handles missing postal_code in address', () => {
      ...
      const { container } = render(
        <SchemaScript vetCenter={dataWithoutPostalCode} />
      )
    ...
    test('handles null office hours gracefully', () => {
      ...
      const { container } = render(
        <SchemaScript vetCenter={dataWithNullHours} />
      )
      ...
      // The dayOfWeek format is https://schema.org/{dayNumber}
      // where day 0 = Sunday
