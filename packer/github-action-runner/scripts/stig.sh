...
migrate_and_mount_disk() {
    ...
    if [ -d "${folder_path}" ]; then
        ...
        sleep 5 # avoid possible race condition on the file system
...
partition_disks() {
    ...
    parted -a optimal -s "${disk_name}" \
        mklabel gpt \
        mkpart var xfs 0% 10% \
        mkpart varlog xfs 10% 20% \
        mkpart varlogaudit xfs 20% 45% \
        mkpart home xfs 45% 50% \
        mkpart varlibdocker xfs 50% 90%
